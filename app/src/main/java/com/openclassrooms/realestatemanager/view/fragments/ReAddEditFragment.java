...
public class ReAddEditFragment extends BaseFragment<FragmentReAddEditBinding> {
    ...
    private boolean mIsMandatoryDataCompletePreviousState = false;
    ...
    @Override
    protected void configureDesign(FragmentReAddEditBinding pBinding, boolean pIsTablet) {
        ...
        mBinding.fragReAddEditCbSold.setOnCheckedChangeListener((buttonView, isChecked) -> {
            if (!isChecked) {
                mBinding.fragReAddEditEtSoldDate.setText(null);
            }
        });
    }
    ...
    private void manageRealEstate() {
        ...
        if (!lIsValidAgentFirstName || !lIsValidAgentLastName) {
            ...
            Toast.makeText(mContext, getString(R.string.add_edit_txt_err_check_sold_not_confirm) + " "
                    + getString(R.string.default_txt_data_not_saved), Toast.LENGTH_SHORT).show();
        } else if (mDateMarket != null || mDateSold != null || lIsSold || !lAgentLastName.equals("")
    ...
    private boolean manageLocation() {
        ...
        if (lErrorData.length() > 0) {
            Toast.makeText(mContext, getString(R.string.add_edit_txt_err_data_incorrect) + lErrorData + " "
                    + getString(R.string.default_txt_data_not_saved), Toast.LENGTH_SHORT).show();
            return false;
        } else {
    ...
    private void navigateTo() {
        mCallback = (OnSaveListener) mContext;
        mCallback.navigateToList();
    }
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        if (mIsEdit) {
            mViewModel.selectReComplete(mReId).observe(getViewLifecycleOwner(), pReComp -> {
                mReComp = pReComp;
                mIsMandatoryDataCompletePreviousState = pReComp.getRealEstate().isReIsMandatoryDataComplete();
    ...
    private void displayReComplete(RealEstateComplete pReComp) {
        ...
        try {
            ...
            pE.printStackTrace();
            Log.d(TAG, "displayReComplete: " + pE.getMessage());
        }
    }
